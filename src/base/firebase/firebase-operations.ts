...
export const putObject = <Type extends object | null>(path: string, obj: Type): Promise<void> =>
  set(ref(db, path), obj);

export const getObject = <Type extends object | null>(path: string): Promise<Type> =>
  get(child(ref(db), path)).then(snapshot => snapshot.val());
